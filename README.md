...
| GitHub Alerts | 5種（Note / Tip / Important / Warning / Caution）。**編集中も色付きで表示**され、`/` やブロックハンドルの ＋ から挿入できる。閲覧モードではGitHubと同じアイコン付きで描画 |
...
| 目次 | 見出し階層を示す細いレールを常時表示し、ホバー/フォーカスで見出しテキストまで展開。クリックでジャンプ、スクロールに追従して現在地を表示（編集・閲覧どちらのモードでも動作） |
...
- **編集モード**: ProseMirrorのDecorationで、条件を満たす引用ブロックに色と枠を与えるだけ。
  マーカー（`[!NOTE]`）はMarkdownの実体なので消さず、見出しとして目立たせる
- **閲覧モード**: GitHubと同じラベル・アイコン・配色で描画（`packages/core/src/segments.ts`）。
  リスト内にインデントされたAlert、複数段落・リストを含むAlertにも対応
...
- **コードブロックの配色は `globals.css` のCSS変数1か所**（`--code-fg` / `--code-keyword` など、
  GitHubのlight/dark配色）で決めている。閲覧モードのhighlight.jsと編集モードのCodeMirrorが
  同じ変数を参照するので、両モード・両テーマで見た目が揃う。
...
- 編集モードと閲覧モードで同じ見出しスタイルを使う（`.docvault-editor` スコープを1段かぶせて
...
こうすると **閲覧モード（react-markdown）と編集モード（ProseMirror、id属性なし）を
同じロジックで扱える** ため、slug生成の一致を気にする必要がありません。
...
- 閲覧時: フェンス考慮の行単位セグメント分割（`packages/core/src/segments.ts`）で
...
- E2E: GitHub REST APIをインメモリモックしたPlaywrightスモークテストで、
  ...
  DBビュー → 検索 → 移動・リネーム時のリンク/画像パス書き換え → 未参照画像削除 →
  テンプレート作成 → デバウンスpush までを検証済み

## 既知の制約

- WYSIWYG編集中のMermaid・埋め込みはソース表示（閲覧モードで描画）。
