...
packages/core   # 純粋ロジック: GitHub APIクライアント / frontmatter / ドキュメント名 /
                # Alerts / 本文セグメント分割 / リンク書き換え / ブロックID /
                # 未参照画像検出 / パス解決（vitestユニットテスト付き）
...
| GitHub Alerts | 5種（Note / Tip / Important / Warning / Caution）。**編集中も色付きで表示**され、ツールバーのボタンで挿入できる。閲覧モードではGitHubと同じアイコン付きで描画 |
...
**ファイル名に一本化しています。** `title` は名前としては使いません。
...
- 新規作成時に frontmatter へ `title` は書き込まない（本文の先頭H1は、GitHub単体で読んだときの
  見出しとして残す）
- リネームは3経路（ページ上部のタイトル欄・ツリーの鉛筆/ダブルクリック・移動ダイアログ）から可能で、
  すべて同じ `moveDoc` に集約される → 参照リンクの書き換えと1コミット反映が常に働く
- 既存ファイルの `title` は消さない。ただし**旧ファイル名と完全に一致していた場合に限り**、
  リネーム時に `title` と先頭H1も追従させる（同期していたものは同期したまま、
  ユーザーが独自に書き換えた見出しには触らない → `packages/core/src/names.ts`）
- 残っている `title` は「普通のプロパティのひとつ」として扱われ、データベースビューの列にもなる。
  プロパティパネルで `title` を持つドキュメントを開くと、表示に使われない旨を明示する
...
- `packages/core`: vitestユニットテスト53件（リンク書き換え・パス解決・ブロック置換・
  frontmatter・ドキュメント名の追従・Alertsの正規化とセグメント分割・未参照画像検出）
...
- ドキュメント名（ファイル名）と本文先頭のH1は別物。新規作成時は一致し、リネーム時も
  一致していれば追従するが、H1を独自に書き換えた後はそれぞれ独立する
